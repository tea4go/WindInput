...
1. 读到 `.yaml` 回退文件 → **v0**，走 `migrateV0toV1` + 格式迁移；
2. 读到 `.toml` 且 `version` 缺失 → **按 v1 处理**（安全默认）。因为 v0 只可能是 YAML，TOML 缺 version 必然是用户手编时误删——不执行任何迁移，下次 diff-save 自动补写 `version = 1`。这天然规避了"误删 version 导致 v1 文件被当 v0 重迁移"的风险，无需特征键探针；
3. `.toml` 且 `version = N`（1 ≤ N ≤ current）→ 从 N 起执行迁移链；
...
`migrateV0toV1` 内容 = §6 映射表逐条搬键 + 熔合现有四个启发式迁移（`migrateQuickInputConfig`、`migrateStatusIndicatorConfig`、`theme:"dark"`、`FontSizeFollowTheme` 缺失=false 的老用户语义）。迁移后 `ApplyConfigFallbacks` 只剩值域 clamp 类兜底，所有 `migrate*` 函数删除。
...
| ui.mode_accent_border | `*bool` | nil=开 | 二态 | 值 bool 默认 true |
...
| ui.font_size / font_size_follow_theme | ui.candidate.* | follow 缺失且文件存在（老用户）→ 显式 false（保留现字号），熔合现有探针语义 |
