...
// （buffer 去掉首字母 z）。决策器开/关下应逐字节等价（golden 回归；决策器已是唯一路径）。
...
// 注：z 键混合回退（judgeZFallback）的「首次退格撤销夺取」由 coordinator 的 z_rewind_test
...
// 链上 navKeyHandler 分发，与旧 handlePinyinModeKey switch 逐字节等价（golden 回归；
// 决策器已是唯一路径）。backtick 进临时拼音、shi 得多候选；PageDown/PageUp
