...
| `tooltip.go` | Tooltip（编码提示）窗口管理 + 事件；`render` 走盒模型 View 引擎（见 viewbox_tooltip.go）；含 `splitLines`/`truncateLineToWidth`/`itoaCompact` 文本辅助。**Host render**：`SetHostRenderFunc(func(img,x,y), hide)` 非 nil 时 `Show/Hide/ForceHide` 把 bitmap 经 SHM 送宿主进程 tooltip band 窗口（高 Band 下压在候选之上不被遮挡），本地窗口隐藏（`hostVisible` 镜像）；由 `Manager.SetTooltipHostRenderFunc` 接线（见 coordinator `updateHostRenderState`），纯显示无鼠标交互 |
