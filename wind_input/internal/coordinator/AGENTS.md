...
| `pipeline_nav.go` / `pipeline_nav_test.go` | **共享候选窗导航**：`navPageUp`/`navPageDown`(showUI,expand,expandBefore)/`navHighlightUp`/`navHighlightDown`(showUI,expand)——消除四套 handleXxxKey 重复的翻页/高亮移动，差异走回调（各模式 showXxxUI + 分级加载 expand，expand 内部自检 hasMore）。已接入 pinyin_mode_shared/quick_input/special（page+highlight）与 temp_english（**仅 page**；其 highlight 因 showUI 无条件刷新 + expand 时序不同仍内联）。辅助函数抽取（字节级等价）；链级分发由 `pipeline_nav_handler.go` 接入（temp_pinyin 已试点） |
| `pipeline_nav_handler.go` / `pipeline_nav_handler_test.go` | **共享导航的链上 KeyHandler（链分解第一步，temp_pinyin 试点）**：`isPinyinModeNavKey`（翻页/高亮谓词，与 `handlePinyinModeKey` 导航 case 逐一致）+ `pinyinNavKeyHandler`（Judge 导航键 Handle/其余 Pass，Apply 调 `navPageUp`/`navPageDown`/`navHighlightUp`/`navHighlightDown`，showUI 走注入的 `ops`）。与「整模式 handler 对导航键 Pass」配对，保证 I11 单一归属、与旧 switch 逐字节等价。单测验证翻页推进 + Pass/Handle 配对 |
...
| `pipeline_temp_pinyin.go` | 临时拼音宿主 `tempPinyinProcessor`：`Judge` 触发键激活裁决（标点触发键经 `matchTempPinyinTrigger`；**z 首次触发经 `judgeZFirstTrigger`** 收编——复用旧 `getTempPinyinTriggerKey` 的 z 渐进仲裁，逐字节等价）/ `Activate` 复用 `setupTempPinyinMode`（z fallback 走 `enterTempPinyinFromZBuffer`）/ `BufferText`=tempPinyinBuffer / `Capabilities`=CapPinyinLayer / `KeyHandlers`=`[tempPinyinKeyHandler, pinyinNavKeyHandler]`（**KeyHandler 链分解试点**：`tempPinyinKeyHandler` 模式特有键 Apply=`handleTempPinyinKey`，但 Judge 对导航键 **Pass** 让位；翻页/高亮由链上居后的 `pinyinNavKeyHandler` 认领，见 `pipeline_nav_handler.go`。其余三模式仍整模式薄包装，待后续批次同样分解） |
