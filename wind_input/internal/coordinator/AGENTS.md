...
| `pipeline_quick_input.go` | 快捷输入宿主 `quickInputProcessor`：`Judge` 触发键激活裁决 / `Activate` 复用 `setupQuickInputMode` / `KeyHandlers`=`[quickInputKeyHandler]`（**整模式薄包装**：Judge 恒 Handle、Apply=`handleQuickInputKey`，含拼音上下文（`quickInputPinyinActive` 派生）内部分发，与旧逐条等价）。**已全接管**（同 temp_pinyin） |
