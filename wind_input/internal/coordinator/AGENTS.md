...
| `pipeline_nav_handler.go` / `pipeline_nav_handler_test.go` | **共享导航的链上 KeyHandler（链分解，已接入 temp_pinyin + special）**：`isStandardNavKey`（翻页/高亮谓词；pinyin 与 special 同谓词 isPageUpKey/isPageDownKey/isHighlightUpKey/isHighlightDownKey；quick_input 基础上下文用专用 isQuickInputPageUpKey，不在此列）+ 通用 `navKeyHandler`（Judge 导航键 Handle/其余 Pass，Apply 调 `navPageUp`/`navPageDown`/`navHighlightUp`/`navHighlightDown`）。**宿主差异经构造参数注入**：`showUI`（showPinyinModeUI(ops) / showSpecialUI）/ `pageDownExpand`（pinyin=nil、special=expandSpecialCandidates）/ `hiDownExpand`。**同一 navKeyHandler 类型被多宿主复用**（true sharedNav）。与「整模式 handler 对导航键 Pass」配对，保 I11 单一归属、与旧 switch 逐字节等价。单测（翻页推进 + Pass/Handle 配对）+ e2e（`mode_temp_pinyin_paging`/`special_highlight_nav` A/B） |
