...
func (c *Coordinator) HandleSelectionChanged(prevChar rune) {
	...
	// IME 自提交后宿主立即回送 SelectionChanged（实测 < 50ms）；grace window 内的事件
	// 视为自提交副作用，不作为真实用户光标移动处理。200ms 留出余量。
	const selfCommitGracePeriod = 200 * time.Millisecond
	isUserDrivenMove := c.lastSelfCommitTime.IsZero() || time.Since(c.lastSelfCommitTime) > selfCommitGracePeriod

	// 智能符号：真实用户光标移动（鼠标点击 / 方向键）解除武装，防止 prevChar=0 时
	// 在新位置误发 ReplaceBackward 删除非目标字符。IME 自提交后的 grace 内不解除，
	// 以保留双击窗口。
	if isUserDrivenMove {
		c.disarmSmartSymbol()
	}
